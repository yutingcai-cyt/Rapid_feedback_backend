...
public interface AssessDao {
    ...
    @Select("SELECT m.id, m.first_name, m.last_name, a.assessed_date, c.name, a.score, a.comment FROM (assessment a JOIN marker m ON a.marker_id = m.id)" +
            "JOIN criteria c ON a.criteria_id = c.id WHERE a.proj_id = #{projectId} AND a.student_id = #{studentId};")
